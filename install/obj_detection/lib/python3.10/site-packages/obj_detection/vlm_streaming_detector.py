...
class VLMStreaming(Node):
    def __init__(self):
        ...
        self.declare_parameter('use_bcr_bot', False)
        use_bcr_bot = self.get_parameter('use_bcr_bot').value
        prefix = "/bcr_bot" if use_bcr_bot else ""
        depth_topic = f"{prefix}/kinect_camera/depth/image_raw"
        color_topic = f"{prefix}/kinect_camera/image_raw"
        info_topic  = f"{prefix}/kinect_camera/camera_info" if use_bcr_bot else "/kinect_camera/depth/camera_info"
        self.get_logger().info(f"Camera topics: color={color_topic}, depth={depth_topic}, info={info_topic}")
